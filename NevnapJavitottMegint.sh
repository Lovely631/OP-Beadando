...
#A t, mint today.
#A n, mint name.
#A d, mint date. pl: 01/01 formatumban

while getopts 'tn:d:' option; do
	case "$option" in
	   t)  t=$(grep `date +%m/%d` $0)
		echo "A mai nevnapok:"
		echo "$t";;
	   n)  n=$(grep $OPTARG $0 | cut -d ' ' -f 1)
		echo "$OPTARG nevnapja/nevnapjai:"
		echo "$n";;
      	   d)  d=$(grep $OPTARG $0 | cut -d ' ' -f 2-)
		echo "$OPTARG napon nevnapja van:"
		echo "$d";;
	esac
done
